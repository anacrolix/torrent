...
// Download represents the state of a torrent within a client.
...
type Download interface {
	...
	SetMaxEstablishedConns(max int) (oldMax int)
}

// download maintains state of torrent within a Client.
type torrent struct {
...
// Torrent provides enough information to lookup the torrent again.
func (t *torrent) Torrent() Torrent {
	return Torrent{
...
func (t *torrent) addPeer(p Peer) {
	...
	if cl.badPeerIPPort(p.IP, p.Port) {
		torrentVar.Add("peers not added because of bad addr", 1)
		return
	}
	if t.peers.Add(p) {
		torrentVar.Add("peers replaced", 1)
	}
	t.openNewConns()
	for t.peers.Len() > cl.config.TorrentPeersHighWater {
		_, ok := t.peers.DeleteMin()
		if ok {
			torrentVar.Add("excess reserve peers discarded", 1)
		}
	}
}
...
// The current working name for the torrentVar. Either the name in the info dict,
...
// Returns true if connection is removed from torrentVar.Conns.
func (t *torrent) deleteConnection(c *connection) (ret bool) {
	...
	_, ret = t.conns[c]
	delete(t.conns, c)
	torrentVar.Add("deleted connections", 1)
	c.deleteAllRequests()
	if len(t.conns) == 0 {
...
// Returns whether the client should make effort to seed the torrentVar.
...
// The total number of peers in the torrentVar.
...
func (t *torrent) addConnection(c *connection) (err error) {
	defer func() {
		if err == nil {
			torrentVar.Add("added connections", 1)
		}
	}()
	if t.closed.IsSet() {
