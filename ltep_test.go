...
func TestUserLtep(t *testing.T) {
	...
	evensCfg.Callbacks.ReadExtendedHandshake = func(pc *PeerConn, msg *pp.ExtendedHandshakeMessage) {
		...
		go func() {
			...
			qt.Check(c, qt.IsNotNil(pc.WriteExtendedMessage("pm_me_floats", []byte("3.142"))))
			// Kick things off by sending a 1.
			qt.Check(c, qt.IsNil(pc.WriteExtendedMessage(testRepliesToOddsExtensionName, []byte("1"))))
		}()
	}
