...
func serve_muti() (cmd bargle.Command) {
	...
	cmd.DefaultAction = func() error {
		...
		totalLength, err := mutiFileTotalLength(filePaths...)
		if err != nil {
			return fmt.Errorf("calculating total length of %q: %v", filePaths, err)
		}
		pieceLength := metainfo.ChoosePieceLength(totalLength)
		info := metainfo.Info{
			PieceLength: pieceLength,
		}
		err = info.BuildFromMutiFilePaths(filePaths...)
		if err != nil {
			return fmt.Errorf("building info from path %q: %w", filePaths, err)
		}
