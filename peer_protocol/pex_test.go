...
func TestUnmarshalPex(t *testing.T) {
	...
	qt.Assert(t, qt.Equals(len(pem.Added), 2))
	qt.Assert(t, qt.Equals(pem.Added[0].Port, 1286))
	qt.Assert(t, qt.Equals(pem.Added[1].Port, 0x100*0xb+0xc))
}
...
func TestMarshalPexMessage(t *testing.T) {
	...
	qt.Assert(t, qt.IsNil(err))
	qt.Assert(t, qt.DeepEquals(expected, b))
	...
	err = dec.Decode(&msg)
	qt.Assert(t, qt.IsNil(err))
	qt.Assert(t, qt.Equals(msg.Type, Extended))
	qt.Assert(t, qt.Equals(msg.ExtendedID, pexExtendedId))
	err = bencode.Unmarshal(msg.ExtendedPayload, &pmOut)
	qt.Assert(t, qt.IsNil(err))
	qt.Assert(t, qt.Equals(len(pmOut.Added), len(pm.Added)))
	qt.Assert(t, qt.DeepEquals(pmOut.Added[0].IP, pm.Added[0].IP))
	qt.Assert(t, qt.Equals(pmOut.Added[0].Port, pm.Added[0].Port))
}
